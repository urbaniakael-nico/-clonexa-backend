...
translator_path = Path("app/web/client_google_translate.js")
...
translator_path.write_text(js, encoding="utf-8")

# Quitar el traductor interno anterior del HTML para que no interfiera.
html = re.sub(
...
if matches:
    last = matches[-1]
    src = last.group(1)
    translator_src = re.sub(r'client\.js(?:\?[^"\']*)?', 'client_google_translate.js?v=020A7', src)
    html = html[:last.end()] + f'\n<script src="{translator_src}"></script>\n' + html[last.end():]
else:
    tag = '\n<script src="/client-static/client_google_translate.js?v=020A7"></script>\n'
...
print("PATCH_OK: Google-style translator bridge added.")
