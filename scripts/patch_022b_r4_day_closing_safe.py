...
safe_endpoint = Path("app/api/v1/endpoints/day_closing_safe.py")

safe_endpoint.write_text(r'''
...
(function clonexaDayClosingSafe022BR4() {
  ...
  async function saveClosing() {
    ...
    try {
      const companyId = companyIdFromUrl();
      const response = await api(`/day-closing-safe/companies/${encodeURIComponent(companyId)}/closures`, {
        method: "POST",
...
html = re.sub(
    r"client_day_closing\.js(?:\?v=[^\"']*)?",
    "client_day_closing.js?v=022BR4",
    html,
    flags=re.IGNORECASE,
)
...
print("PATCH_OK: 022B-R4 safe day closing installed")
