...
js_path = Path("app/web/client_gps_i18n_safe.js")
...
html = re.sub(
    r'\s*<script[^>]+src=["\'][^"\']*client_gps_i18n_safe\.js[^"\']*["\'][^>]*>\s*</script>\s*',
    "\n",
    html,
    flags=re.IGNORECASE,
)

# Insertar después de inventory_i18n_safe si existe; si no, después de dashboard_i18n_safe
...
if inventory_matches:
    ...
    src = last.group(1)
    safe_src = re.sub(r'client_inventory_i18n_safe\.js(?:\?v=[^"\']*)?', 'client_gps_i18n_safe.js?v=020GR1', src)
    ...
    if dashboard_matches:
        ...
        src = last.group(1)
        safe_src = re.sub(r'client_dashboard_i18n_safe\.js(?:\?v=[^"\']*)?', 'client_gps_i18n_safe.js?v=020GR1', src)
        html = html[:last.end()] + f'\n<script src="{safe_src}"></script>\n' + html[last.end():]
    else:
        html = html.replace("</body>", '<script src="/client-static/client_gps_i18n_safe.js?v=020GR1"></script>\n</body>')

html_path.write_text(html, encoding="utf-8")

print("PATCH_OK: 020G-R1 safe external GPS i18n added")
