...
class WorkforceAttendanceEvent(Base):
    ...
    # 010B-R2 bitácora operativa: bot/panel/QR/módulo origen/evidencia.
    module_code: Mapped[str] = mapped_column(String(80), nullable=False, server_default=text("'workforce'"), index=True)
    source_channel: Mapped[str] = mapped_column(String(80), nullable=False, server_default=text("'client'"), index=True)
    source_ref: Mapped[str | None] = mapped_column(String(180), nullable=True)
    bot_instance_id: Mapped[UUID | None] = mapped_column(PG_UUID(as_uuid=True), nullable=True)
    ...
    occurred_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, server_default=text("now()"), index=True)
