...
def parse_telegram_update(company_id: UUID, update: dict[str, Any]) -> EventCreate | None:
    ...
    if text == "ðŸŸ¢ Iniciar turno" or text.lower() in {"/start_shift", "iniciar turno"}:
        event_type = "shift_started"
    elif text == "â¸ Pausa" or text.lower() in {"pausa", "/pause"}:
        event_type = "shift_paused"
    elif text == "â–¶ï¸ Reanudar" or text.lower() in {"reanudar", "/resume"}:
        event_type = "shift_resumed"
    elif text == "ðŸ”´ Finalizar jornada" or text.lower() in {"finalizar", "/end_shift"}:
