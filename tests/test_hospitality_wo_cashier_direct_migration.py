...
MIGRATION_PATH = (
    ...
    / "migrations" / "versions" / "021h_wo_cashier_direct.py"
)
...
def load_migration():
    spec = importlib.util.spec_from_file_location("wo_cashier_direct", MIGRATION_PATH)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


def test_revision_id_fits_and_chains_onto_021g():
    module = load_migration()
    assert len(module.revision) <= 32
    assert module.down_revision == "021g_wo_kitchen_qty_btns"
...
def test_upgrade_turns_the_switch_on_for_asadero_el_socio_only(monkeypatch):
    ...
    assert json.loads(module.SETTINGS_PATCH) == {"cashier_direct_sale": True}
...
def test_downgrade_removes_only_that_key(monkeypatch):
    ...
    module.downgrade()
    joined = "\n".join(fake_op.statements)
    assert "- 'cashier_direct_sale'" in joined
