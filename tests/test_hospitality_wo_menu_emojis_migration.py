...
MIGRATION_PATH = (
    ...
    / "migrations" / "versions" / "021j_wo_menu_emojis.py"
)
...
def load_migration():
    spec = importlib.util.spec_from_file_location("wo_menu_emojis", MIGRATION_PATH)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


def test_revision_id_fits_and_chains_onto_021i():
    module = load_migration()
    assert len(module.revision) <= 32
    assert module.down_revision == "021i_wo_kitchen_roster"
...
def test_upgrade_turns_the_switch_on_for_asadero_el_socio_only(monkeypatch):
    ...
    assert json.loads(module.SETTINGS_PATCH) == {"menu_emojis": True}
